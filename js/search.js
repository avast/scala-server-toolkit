...
function prepareIdxAndDocMap() {
  const docs = [  
    ...
    } ,    
    {
      ...
      "content": "Datastax Cassandra Driver This subproject initializes Datastax Cassandra driver’s Session. libraryDependencies += \"com.avast\" %% \"sst-cassandra-datastax-driver\" % \"0.18.4\" import cats.effect.Resource import com.avast.sst.datastax.CassandraDatastaxDriverModule import com.avast.sst.datastax.config.CassandraDatastaxDriverConfig import com.avast.sst.datastax.pureconfig.implicits.* import com.avast.sst.pureconfig.PureConfigModule import zio.* import zio.interop.catz.* implicit val runtime: Runtime[ZEnv] = zio.Runtime.default // this is just needed in example for { configuration &lt;- Resource.eval(PureConfigModule.makeOrRaise[Task, CassandraDatastaxDriverConfig]) db &lt;- CassandraDatastaxDriverModule.make[Task](configuration) } yield db basic { contact-points = [\"localhost:9042\"] load-balancing-policy { local-datacenter = \"datacenter1\" } }"
    } ,    
    {
      "title": "Doobie",
      "url": "/scala-server-toolkit/subprojects/doobie.html",
      "content": "Doobie libraryDependencies += \"com.avast\" %% \"sst-doobie-hikari\" % \"0.18.4\" This subproject initializes a doobie Transactor: import cats.effect.Resource import com.avast.sst.doobie.DoobieHikariModule import com.avast.sst.example.config.Configuration import com.avast.sst.jvm.execution.ConfigurableThreadFactory.Config import com.avast.sst.jvm.execution.{ConfigurableThreadFactory, ExecutorModule} import com.avast.sst.micrometer.jmx.MicrometerJmxModule import com.avast.sst.pureconfig.PureConfigModule import com.zaxxer.hikari.metrics.micrometer.MicrometerMetricsTrackerFactory import scala.concurrent.ExecutionContext import zio.* import zio.interop.catz.* implicit val runtime: Runtime[ZEnv] = zio.Runtime.default // this is just needed in example for { configuration &lt;- Resource.eval(PureConfigModule.makeOrRaise[Task, Configuration]) executorModule &lt;- ExecutorModule.makeFromExecutionContext[Task](runtime.platform.executor.asEC) meterRegistry &lt;- MicrometerJmxModule.make[Task](configuration.jmx) boundedConnectExecutionContext &lt;- executorModule .makeThreadPoolExecutor( configuration.boundedConnectExecutor, new ConfigurableThreadFactory(Config(Some(\"hikari-connect-%02d\"))) ) .map(ExecutionContext.fromExecutorService) hikariMetricsFactory = new MicrometerMetricsTrackerFactory(meterRegistry) doobieTransactor &lt;- DoobieHikariModule .make[Task](configuration.database, boundedConnectExecutionContext, executorModule.blocker, Some(hikariMetricsFactory)) } yield doobieTransactor"
    } ,    
    {
      "title": "Flyway",
      "url": "/scala-server-toolkit/subprojects/flyway.html",
      "content": "Flyway libraryDependencies += \"com.avast\" %% \"sst-flyway\" % \"0.18.4\" This subproject initializes Flyway which can be used to do automated SQL DB migrations. See the documentation of Flyway on how to go about that. The method make requires javax.sql.DataSource which you can for example obtain from doobie-hikari subproject: import cats.effect.Resource import com.avast.sst.doobie.DoobieHikariModule import com.avast.sst.flyway.FlywayModule import zio.Task import zio.interop.catz.* for { doobieTransactor &lt;- DoobieHikariModule.make[Task](???, ???, ???, ???) flyway &lt;- Resource.eval(FlywayModule.make[Task](doobieTransactor.kernel, ???)) _ &lt;- Resource.eval(Task.effect(flyway.migrate())) } yield ()"
    } ,    
    {
      "title": "FS2 Kafka",
      "url": "/scala-server-toolkit/subprojects/fs2-kafka.html",
      "content": "FS2 Kafka libraryDependencies += \"com.avast\" %% \"sst-fs2-kafka\" % \"0.18.4\" This subproject initializes FS2 Kafka consumer or producer: import cats.effect.Resource import cats.syntax.flatMap.* import com.avast.sst.fs2kafka.* import fs2.kafka.{AutoOffsetReset, ProducerRecord, ProducerRecords} import zio.* import zio.interop.catz.* import zio.interop.catz.implicits.* implicit val runtime: Runtime[ZEnv] = zio.Runtime.default // this is just needed in example for { consumer &lt;- Fs2KafkaModule.makeConsumer[Task, String, String]( ConsumerConfig(List(\"localhost:9092\"), groupId = \"test\", autoOffsetReset = AutoOffsetReset.Earliest), None, None ) _ &lt;- Resource.eval(consumer.subscribeTo(\"test\")) consumerStream &lt;- Resource.eval(consumer.stream) } yield consumerStream The configuration of Kafka client is very large therefore you can either use the provided configuration case class, or you can use the underlying ConsumerSettings/ProducerSettings builders directly. The configuration case classes contain an “escape hatch” into the full world of Kafka client configuration options via untyped properties. This is there to be flexible in case it is needed. Documentation of all the configuration properties is available here: consumer producer Beware that there is an optional dependency on jackson-databind for the default implementation of SASL/OAUTHBEARER in kafka-clients. You need to provide it explicitly: https://kafka.apache.org/documentation/#security_sasl_oauthbearer_clientconfig"
    } ,    
    {
      "title": "Getting Started",
      "url": "/scala-server-toolkit/getting-started.html",
      "content": "Getting Started Creating a simple HTTP server using http4s and ZIO is as easy as this: build.sbt libraryDependencies += \"com.avast\" %% \"sst-bundle-zio-http4s-blaze\" % \"0.18.4\" Main import cats.effect.* import com.avast.sst.http4s.client.* import com.avast.sst.http4s.server.* import com.avast.sst.jvm.execution.ExecutorModule import com.avast.sst.jvm.system.console.ConsoleModule import org.http4s.dsl.Http4sDsl import org.http4s.HttpRoutes import zio.interop.catz.* import zio.interop.catz.implicits.* import zio.{Task, ZEnv, Runtime} implicit val runtime: Runtime[ZEnv] = zio.Runtime.default // this is just needed in example val dsl = Http4sDsl[Task] // this is just needed in example import dsl.* val routes = Http4sRouting.make { HttpRoutes.of[Task] { case GET -&gt; Root / \"hello\" =&gt; Ok(\"Hello World!\") } } val resource = for { executorModule &lt;- ExecutorModule.makeDefault[Task] console = ConsoleModule.make[Task] server &lt;- Http4sBlazeServerModule.make[Task](Http4sBlazeServerConfig(\"127.0.0.1\", 0), routes, executorModule.executionContext) client &lt;- Http4sBlazeClientModule.make[Task](Http4sBlazeClientConfig(), executorModule.executionContext) } yield (server, client, console) val program = resource .use { case (server, client, console) =&gt; client .expect[String](s\"http://127.0.0.1:${server.address.getPort}/hello\") .flatMap(console.printLine) } runtime.unsafeRun(program) // Hello World! Or you can use the official Giter8 template: sbt new avast/sst-seed.g8"
    } ,    
    {
      "title": "http4s",
      "url": "/scala-server-toolkit/subprojects/http4s.html",
      "content": "http4s libraryDependencies += \"com.avast\" %% \"sst-http4s-server-blaze\" % \"0.18.4\" There are multiple http4s-* subprojects that provide easy initialization of a server and a client. Http4s is an interface with multiple possible implementations - for now we provide only implementations based on Blaze. Both server and client are configured via configuration case class which contains default values taken from the underlying implementations. import cats.effect.* import com.avast.sst.http4s.client.* import com.avast.sst.http4s.server.* import com.avast.sst.jvm.execution.ExecutorModule import com.avast.sst.jvm.system.console.ConsoleModule import org.http4s.dsl.Http4sDsl import org.http4s.HttpRoutes import zio.interop.catz.* import zio.interop.catz.implicits.* import zio.* implicit val runtime: Runtime[ZEnv] = zio.Runtime.default // this is just needed in example val dsl = Http4sDsl[Task] // this is just needed in example import dsl.* val routes = Http4sRouting.make { HttpRoutes.of[Task] { case GET -&gt; Root / \"hello\" =&gt; Ok(\"Hello World!\") } } val resource = for { executorModule &lt;- ExecutorModule.makeDefault[Task] console = ConsoleModule.make[Task] server &lt;- Http4sBlazeServerModule.make[Task](Http4sBlazeServerConfig(\"127.0.0.1\", 0), routes, executorModule.executionContext) client &lt;- Http4sBlazeClientModule.make[Task](Http4sBlazeClientConfig(), executorModule.executionContext) } yield (server, client, console) val program = resource .use { case (server, client, console) =&gt; client .expect[String](s\"http://127.0.0.1:${server.address.getPort}/hello\") .flatMap(console.printLine) } runtime.unsafeRun(program) // Hello World! Middleware Correlation ID Middleware import cats.effect.* import com.avast.sst.http4s.server.* import com.avast.sst.http4s.server.middleware.CorrelationIdMiddleware import com.avast.sst.jvm.execution.ExecutorModule import org.http4s.dsl.Http4sDsl import org.http4s.HttpRoutes import zio.interop.catz.* import zio.interop.catz.implicits.* import zio.* val dsl = Http4sDsl[Task] // this is just needed in example import dsl.* implicit val runtime: Runtime[ZEnv] = zio.Runtime.default // this is just needed in example for { middleware &lt;- Resource.eval(CorrelationIdMiddleware.default[Task]) executorModule &lt;- ExecutorModule.makeDefault[Task] routes = Http4sRouting.make { middleware.wrap { HttpRoutes.of[Task] { case GET -&gt; Root =&gt; // val correlationId = middleware.retrieveCorrelationId(req) ??? } } } server &lt;- Http4sBlazeServerModule.make[Task](Http4sBlazeServerConfig.localhost8080, routes, executorModule.executionContext) } yield server Circuit Breaker It is a good practice to wrap any communication with external system with circuit breaking mechanism to prevent spreading of errors and bad latency. See monix-catnap for one of the options."
    } ,    
    {
      "title": "Home",
      "url": "/scala-server-toolkit/",
      "content": "This project is a culmination of years of Scala development at Avast and tries to represent the best practices of Scala server development we have gained together with tools that allow us to be effective. It is a set of small, flexible and cohesive building blocks that fit together well and allow you to build reliable server applications. Jump Right In You can use the official Giter8 template to get started: sbt new avast/sst-seed.g8 Or read documentation or deep dive into example code. SBT Dependency libraryDependencies += \"com.avast\" %% \"sst-bundle-zio-http4s-blaze\" % \"0.18.4\" Resources Articles Introducing Scala Server Toolkit (Avast Engineering) SST - Creating HTTP Server (Avast Engineering) Talks Intro to Scala Server Toolkit (Functional JVM Meetup, Prague, January 23, 2020) slides"
    } ,    
    {
      "title": "JVM",
      "url": "/scala-server-toolkit/subprojects/jvm.html",
      "content": "JVM libraryDependencies += \"com.avast\" %% \"sst-jvm\" % \"0.18.4\" Subproject sst-jvm provides pure implementations of different JVM-related utilities: creation of thread pools, standard in/out/err, and random number generation. import com.avast.sst.jvm.system.console.ConsoleModule import com.avast.sst.jvm.system.random.RandomModule import zio.interop.catz.* import zio.Task val program = for { random &lt;- RandomModule.makeRandom[Task](1234L) // do not ever use seed like this! randomNumber &lt;- random.nextInt console = ConsoleModule.make[Task] _ &lt;- console.printLine(s\"Random number: $randomNumber\") } yield () val runtime = zio.Runtime.default // this is just needed in example runtime.unsafeRun(program) // Random number: -1517918040"
    } ,    
    {
      "title": "Lettuce (Redis)",
      "url": "/scala-server-toolkit/subprojects/lettuce.html",
      "content": "Lettuce (Redis) libraryDependencies += \"com.avast\" %% \"sst-lettuce\" % \"0.18.4\" This subproject initializes Lettuce Redis driver: import cats.effect.Resource import com.avast.sst.lettuce.{LettuceConfig, LettuceModule} import io.lettuce.core.codec.{RedisCodec, StringCodec} import zio.* import zio.interop.catz.* implicit val runtime: Runtime[ZEnv] = zio.Runtime.default // this is just needed in example implicit val lettuceCodec: RedisCodec[String, String] = StringCodec.UTF8 for { connection &lt;- LettuceModule.makeConnection[Task, String, String](LettuceConfig(\"redis://localhost\")) value &lt;- Resource.eval(Task.effect(connection.sync().get(\"key\"))) } yield value"
    } ,    
    {
      "title": "Micrometer",
      "url": "/scala-server-toolkit/subprojects/micrometer.html",
      "content": "Micrometer libraryDependencies += \"com.avast\" %% \"sst-micrometer-jmx\" % \"0.18.4\" This subproject allows you to monitor your applications using Micrometer. There are many actual implementations of the Micrometer API one of which is JMX. Subproject sst-micrometer-jmx implements the initialization of Micrometer for JMX. There are also interop subprojects such as sst-http4s-server-micrometer which implement monitoring of HTTP server and individual routes using Micrometer. import cats.effect.{Clock, Resource} import com.avast.sst.http4s.server.* import com.avast.sst.http4s.server.micrometer.MicrometerHttp4sServerMetricsModule import com.avast.sst.jvm.execution.ExecutorModule import com.avast.sst.jvm.micrometer.MicrometerJvmModule import com.avast.sst.micrometer.jmx.* import org.http4s.dsl.Http4sDsl import org.http4s.HttpRoutes import org.http4s.server.Server import zio.interop.catz.* import zio.interop.catz.implicits.* import zio.* implicit val runtime: Runtime[ZEnv] = zio.Runtime.default // this is just needed in example val dsl = Http4sDsl[Task] // this is just needed in example import dsl.* for { executorModule &lt;- ExecutorModule.makeFromExecutionContext[Task](runtime.platform.executor.asEC) clock = Clock.create[Task] jmxMeterRegistry &lt;- MicrometerJmxModule.make[Task](MicrometerJmxConfig(\"com.avast\")) _ &lt;- Resource.eval(MicrometerJvmModule.make[Task](jmxMeterRegistry)) serverMetricsModule &lt;- Resource.eval(MicrometerHttp4sServerMetricsModule.make[Task](jmxMeterRegistry, executorModule.blocker, clock)) routes = Http4sRouting.make { serverMetricsModule.serverMetrics { HttpRoutes.of[Task] { case GET -&gt; Root / \"hello\" =&gt; Ok(\"Hello World!\") } } } server &lt;- Http4sBlazeServerModule.make[Task](Http4sBlazeServerConfig(\"127.0.0.1\", 0), routes, executorModule.executionContext) } yield server"
    } ,    
    {
      "title": "monix-catnap",
      "url": "/scala-server-toolkit/subprojects/monix-catnap.html",
      "content": "monix-catnap libraryDependencies += \"com.avast\" %% \"sst-monix-catnap\" % \"0.18.4\" This subproject provides interop between Scala Server Toolkit and monix-catnap library. Circuit Breaker You can use CircuitBreakerModule to instantiate and configure a circuit breaker and you can implement CircuitBreakerMetrics to get monitoring of the circuit breaker. There is an implementation for Micrometer in sst-monix-catnap-micrometer subproject. All of this is tied with http4s HTTP client in the sst-http4s-client-monix-catnap subproject so in practice you want to use Http4sClientCircuitBreakerModule which wraps any Client[F] with a CircuitBreaker (it is recommended to have an enriched CircuitBreaker with logging and metrics - see CircuitBreakerModule companion object methods). However the most important feature of the enriched circuit breaker is that any HTTP failure (according to HttpStatusClassifier) is converted to an exception internally which triggers the circuit breaking mechanism. Failing server is not overloaded by more requests and we do not have to wait for the response if the server is failing anyway."
    } ,      
    {
      "title": "PureConfig",
      "url": "/scala-server-toolkit/subprojects/pureconfig.html",
      "content": "PureConfig libraryDependencies += \"com.avast\" %% \"sst-pureconfig\" % \"0.18.4\" This subproject allows you to load your application’s configuration file into a case class. It uses PureConfig library to do so. PureConfig uses Lightbend Config which means that your application’s configuration will be in HOCON format. Loading of a configuration is effectful, so it is wrapped in F[_] which is Sync. This module also tweaks the error messages a little. Usage and typeclass derivation Unfortunately there are some incompatible changes between Scala 2 and Scala 3 series of PureConfig when it comes to typeclass derivation. We’ve created different implementation for each series in order to unblock migration to Scala 3 for SST users. Read on for further details on usage of each. Scala 2 Sample usage: import com.avast.sst.pureconfig.PureConfigModule import pureconfig.ConfigReader import pureconfig.generic.semiauto.deriveReader import zio.interop.catz.* import zio.Task final case class ServerConfiguration(listenAddress: String, listenPort: Int) implicit val serverConfigurationReader: ConfigReader[ServerConfiguration] = deriveReader val maybeConfiguration = PureConfigModule.make[Task, ServerConfiguration] We also provide modules with implicits for many of SST modules. Look for sst-*-pureconfig subprojects to get implicit instances of ConfigReader for specific libraries, e.g.: import com.avast.sst.http4s.server.pureconfig.implicits.* The default *.pureconfig.implicits._ import contains the default naming convention for PureConfig which is kebab-case for the configuration file and camelCase for the case class field names. You can either choose a different naming convention by different import (e.g. *.pureconfig.implicits.CamelCase) or you can create your own and extend the ConfigReaders trait. Scala 3 Unlike for Scala 2, pureconfig in Scala 3 supports only kebab-case derivation. This is reflected in the structure of the implicits we provide. The derivation API has also changed. See official docs for more details. Sample usage: import com.avast.sst.pureconfig.PureConfigModule import pureconfig.ConfigReader import zio.interop.catz.* import zio.Task import pureconfig.generic.derivation.default.* final case class ServerConfiguration(listenAddress: String, listenPort: Int) implicit val serverConfigurationReader: ConfigReader[ServerConfiguration] = ConfigReader.derived val maybeConfiguration = PureConfigModule.make[Task, ServerConfiguration] Toggle Sometimes it is useful to be able to disable some functionality via config. That usually means to add a new boolean field to particular configuration case class. You have to be aware of this new field when you work with the configuration and check the value of it to enable/disable particular functionality. Toggle is a more safe approach for this pattern as it can be either Enabled[T] or Disabled. So it will force you to deal with the possibility of disabled functionality. It also simplifies the workflow and particular config case class as it does not require any changes to it. The boolean flag is build in the Toggle itself and the value is converted to Enabled[T] or Disabled. For more details please see the example below: case class MyConfig(myService: Toggle[ServiceConfig]) case class ServiceConfig(url: String, timeout: Duration) my-service { enabled = false url = \"http://www.example.com/v1/some-endpoint\" timeout = 5s } WithConfig Sometimes you need to load the configuration both as your custom configuration class and raw Config when you deal with legacy code. Using WithConfig case class allows you to load both objects simultaneously. Example: case class MyConfig(myService: WithConfig[ServiceConfig]) case class ServiceConfig(url: String, timeout: Duration) val serviceConfig: Config = myConfig.myService.config val urlFromConfig: String = serviceConfig.getString(\"url\") val serviceConfigCustom: ServiceConfig = myConfig.myService.value val urlFromCustomObject: String = serviceConfigCustom.url"
    } ,    
    {
      "title": "Rationale",
