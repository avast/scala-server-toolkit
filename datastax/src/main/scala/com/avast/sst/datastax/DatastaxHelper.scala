...
object DatastaxHelper {
  ...
  def durationProperty(opt: DriverOption)(value: Duration)(b: DriverBuilder): DriverBuilder = b.withDuration(opt, java.time.Duration.ofNanos(value.toNanos))
  ...
  def intListProperty(opt: DriverOption)(value: List[Int])(b: DriverBuilder): DriverBuilder = b.withIntList(opt, value.map(i => new Integer(i)).asJava)
  def optional[T](f: T => DriverBuilder => DriverBuilder, value: Option[T])(b: DriverBuilder): DriverBuilder = value.map(f(_)(b)).getOrElse(b)
}