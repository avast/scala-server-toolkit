...
object TimeSyntax {
  ...
  final class FOps[F[_], A](private val f: F[A]) extends AnyVal {
    ...
    def time(record: Duration => F[Unit])(implicit F: MonadCancel[F, Throwable], C: Clock[F]): F[A] = TimeUtils.time(f)(record)
    ...
    /** Measures the time it takes the effect to finish and records it using the provided function. It distinguishes between successful
      ...
      */
    def timeCase(record: Either[Duration, Duration] => F[Unit])(implicit F: MonadCancel[F, Throwable], C: Clock[F]): F[A] = {
